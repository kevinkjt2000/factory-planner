...
type RecipeQuery struct {
	...
	withRecipeType *RecipeTypeQuery
...
func (rq *RecipeQuery) QueryRecipeType() *RecipeTypeQuery {
	query := (&RecipeTypeClient{config: rq.config}).Query()
...
func (rq *RecipeQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (rq *RecipeQuery) Clone() *RecipeQuery {
	...
	return &RecipeQuery{
		...
		withRecipeType: rq.withRecipeType.Clone(),
...
func (rq *RecipeQuery) WithRecipeType(opts ...func(*RecipeTypeQuery)) *RecipeQuery {
	query := (&RecipeTypeClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rq.withRecipeType = query
	return rq
}
...
//
//	client.Recipe.Query().
//		GroupBy(recipe.FieldRecipeTypeID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Recipe.Query().
//		Select(recipe.FieldRecipeTypeID).
//		Scan(ctx, &v)
...
func (rq *RecipeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Recipe, error) {
	...
	if query := rq.withRecipeType; query != nil {
		if err := rq.loadRecipeType(ctx, query, nodes, nil,
			func(n *Recipe, e *RecipeType) { n.Edges.RecipeType = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RecipeQuery) loadRecipeType(ctx context.Context, query *RecipeTypeQuery, nodes []*Recipe, init func(*Recipe), assign func(*Recipe, *RecipeType)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Recipe)
	for i := range nodes {
		fk := nodes[i].RecipeTypeID
	...
	query.Where(recipetype.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "recipe_type_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *RecipeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withRecipeType != nil {
			_spec.Node.AddColumnOnce(recipe.FieldRecipeTypeID)
		}
	}
